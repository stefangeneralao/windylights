...
function parseRules(raw: string[]): ScheduleRule[] {
  return raw.flatMap((r) => {
    ...
    // Gen1 uses 1–7 (Mon–Sun); convert 7→0 for app-internal 0=Sunday
    const days = rawDays.split("").map((d) => d === "7" ? "0" : d).join("");
...
function formatRules(rules: ScheduleRule[]): string {
  return rules
    .map((r) => {
      // App uses 0=Sunday; Gen1 expects 7=Sunday
      const days = r.days.split("").map((d) => d === "0" ? "7" : d).join("");
